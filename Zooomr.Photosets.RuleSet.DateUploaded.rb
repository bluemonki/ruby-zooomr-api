...
class ZooomrPhotosetsRuleSetDateUploaded < ZooomrPhotosetsRuleSetBase
  ...
  def initialize(a_parameter_hash)
    ...
    if (!(a_parameter_hash['date'].is_a?(Date)))
      raise ArgumentError, "ZooomrPhotosetsRuleSetDateUploaded: requires a Date object for date", caller
    end
    ...
    @match_type = ZooomrPhotosetsRuleSet::PHOTOSETMATCH_DATEUPLOADED
